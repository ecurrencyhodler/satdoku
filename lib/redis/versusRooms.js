...
/**
 * Clean up abandoned games where both players have been disconnected for 30+ minutes
...
export async function cleanupAbandonedGames() {
  try {
    const redis = await getRedisClient();
    if (!redis) {
      console.error('[versusRooms] Redis not available for cleanup');
      return { deleted: 0, errors: 0 };
    }

    const ABANDONMENT_THRESHOLD_MS = 30 * 60 * 1000; // 30 minutes in milliseconds
    const now = Date.now();
    let deleted = 0;
    let errors = 0;
    ...
    for (const roomKey of keys) {
      try {
        ...
        // Only check rooms that have both players
        if (!room.players?.player1 || !room.players?.player2) {
          continue;
        }
        ...
        if (player1.connected === false && player2.connected === false) {
          // Check if both have been disconnected for 30+ minutes
          const player1DisconnectedAt = player1.lastDisconnectedAt 
            ? new Date(player1.lastDisconnectedAt).getTime() 
            : null;
          const player2DisconnectedAt = player2.lastDisconnectedAt 
            ? new Date(player2.lastDisconnectedAt).getTime() 
            : null;

          // If either player doesn't have a lastDisconnectedAt timestamp, 
          // we can't determine abandonment (might be old data)
          if (!player1DisconnectedAt || !player2DisconnectedAt) {
            continue;
          }

          // Check if both have been disconnected for 30+ minutes
          const player1Abandoned = (now - player1DisconnectedAt) >= ABANDONMENT_THRESHOLD_MS;
          const player2Abandoned = (now - player2DisconnectedAt) >= ABANDONMENT_THRESHOLD_MS;

          if (player1Abandoned && player2Abandoned) {
            // Delete the abandoned room
            const roomId = room.roomId || roomKey.replace(ROOM_PREFIX, '');
            const deletedSuccessfully = await deleteRoom(roomId);
            if (deletedSuccessfully) {
              deleted++;
              console.log(`[versusRooms] Cleaned up abandoned room ${roomId}`);
            } else {
              errors++;
            }
          }
        ...
        console.error(`[versusRooms] Error processing room ${roomKey}:`, error);
        errors++;
      }
    }

    if (deleted > 0 || errors > 0) {
      console.log(`[versusRooms] Cleanup completed: ${deleted} rooms deleted, ${errors} errors`);
    }
