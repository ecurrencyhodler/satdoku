...
import { handleKeepPlayingWithDifficulty } from './actionHandlers/keepPlayingWithDifficulty.js';
...
export class ServerGameController {
  ...
  static async processAction(sessionId, action, expectedVersion = null) {
    ...
    switch (action.action) {
      ...
      case 'keepPlayingWithDifficulty':
        if (!action.difficulty) {
          return {
            success: false,
            error: 'Difficulty is required for keepPlayingWithDifficulty action',
            errorCode: 'INVALID_ACTION'
          };
        }
        // Validate difficulty is one of the allowed values
        const VALID_DIFFICULTIES = ['beginner', 'medium', 'hard'];
        if (!VALID_DIFFICULTIES.includes(action.difficulty)) {
          return {
            success: false,
        ...
        return await handleKeepPlayingWithDifficulty(sessionId, state, action.difficulty, currentVersion);
      case 'purchaseLife':
