...
/**
 * Log trigger to Redis (fire-and-forget, non-blocking)
 */
async function logTriggerToRedis() {
  try {
    const client = await getRedisClient();
    if (!client) {
      // Redis not available, skip logging
      return;
    }
    ...
    // Increment trigger counter (always logged when function is called)
    const triggerKey = `answer_leak:trigger:${dateKey}`;
    await client.incr(triggerKey);
    // Set expiration to 90 days (keep historical data)
    await client.expire(triggerKey, 90 * 24 * 60 * 60);
  } catch (error) {
    // Silently fail - logging should not break the leak detection
    console.error('[answerLeakDetector] Error logging trigger to Redis:', error);
  }
}

/**
 * Log leak detection to Redis (fire-and-forget, non-blocking)
 */
async function logLeakDetectedToRedis() {
  try {
    const client = await getRedisClient();
    if (!client) {
      // Redis not available, skip logging
    ...
    const detectedKey = `answer_leak:detected:${dateKey}`;
    await client.incr(detectedKey);
    // Set expiration to 90 days (keep historical data)
    await client.expire(detectedKey, 90 * 24 * 60 * 60);
  } catch (error) {
    // Silently fail - logging should not break the leak detection
    console.error('[answerLeakDetector] Error logging leak detection to Redis:', error);
  }
}
        ...
        if (mentionedDigits.includes(solutionValue)) {
          ...
          for (const pattern of confirmationPatterns) {
            if (pattern.test(response)) {
              ...
              logLeakDetectedToRedis().catch(() => {}); // Log leak detection
              return true;
            }
          }
