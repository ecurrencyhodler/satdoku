...
export async function handleMdkWebhook(request) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      // Validate event structure before accessing nested properties
      if (!event.data || !event.data.object) {
        console.error('[webhook] Invalid event structure: missing data.object', event);
      ...
      if (!session.id) {
        console.error('[webhook] Invalid session: missing id', session);
        return NextResponse.json(
          { error: 'Invalid session data: missing id' },
          { status: 400 }
        );
      }
