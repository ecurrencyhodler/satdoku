...
async function getRedisClient() {
  if (client && client.isOpen && typeof client.pipeline === 'function') {
    return client;
  }
  ...
  // Reset if client exists but is not open or missing methods
  if (client && (!client.isOpen || typeof client.pipeline !== 'function')) {
    client = null;
    clientPromise = null;
  }
  ...
  if (!clientPromise) {
    ...
    try {
      ...
      // Verify client has required methods after connection
      if (!client || !client.isOpen || typeof client.pipeline !== 'function') {
        console.error('[Redis] Client missing required methods after connection');
        client = null;
        clientPromise = null;
        return null;
      }
    ...
    try {
      await clientPromise;
      // Verify client is still open and has required methods after awaiting
      if (!client || !client.isOpen || typeof client.pipeline !== 'function') {
        client = null;
        clientPromise = null;
        return null;
...
export async function storeGameState(sessionId, gameState, expectedVersion = null) {
  try {
    ...
    const result = await redis.eval(
      ...
      versionKey,
      key,
      expectedVersion !== null ? expectedVersion.toString() : '',
      stateValue,
      expiration.toString()
    );
...
export async function getGameState(sessionId) {
  try {
    ...
    // Verify pipeline method exists before using it
    if (typeof redis.pipeline !== 'function') {
      console.error('[Redis] Client does not have pipeline method');
      return null;
    }
    ...
    // Get both state and version atomically using pipeline
    const pipeline = redis.pipeline();
    pipeline.get(key);
    pipeline.get(versionKey);
    const results = await pipeline.exec();
    
    const [stateResult, versionResult] = results;
    const value = stateResult[1]; // [error, result] format
    ...
    const versionFromKey = versionResult[1] ? parseInt(versionResult[1], 10) : null;
    parsed.version = versionFromKey !== null ? versionFromKey : (parsed.version ?? 0);
    
    return parsed;
  } catch (error) {
    console.error('[Redis] Error getting game state:', error);
    // Reset client on connection errors or pipeline errors
    if (error.message?.includes('Connection') || error.message?.includes('ECONNREFUSED') || error.message?.includes('pipeline')) {
      client = null;
      clientPromise = null;
    }
    return null;
...
export async function deleteGameState(sessionId) {
  try {
    ...
    // Delete both state and version keys
    const pipeline = redis.pipeline();
    pipeline.del(key);
