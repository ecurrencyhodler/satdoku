...
export async function getRedisClient() {
  ...
  // #region agent log
  console.log(`[Redis Client] getRedisClient called, REDIS_URL set: ${!!redisUrl}, client exists: ${!!client}, client isOpen: ${client?.isOpen}`);
  // #endregion

  if (!redisUrl) {
    // #region agent log
    console.error('[Redis Client] REDIS_URL not set, Redis operations will be disabled');
    console.error('[Redis Client] Available env vars:', Object.keys(process.env).filter(k => k.includes('REDIS')));
    // #endregion
