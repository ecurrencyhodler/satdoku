 ...
 * @returns {Promise<boolean>} - Success status
 */
export async function saveCompletion(sessionId, score, difficulty) {
  try {
    const redis = await getRedisClient();
    if (!redis) {
    ...
    console.log(`[Redis] Saved completion for session: ${sessionId}, score: ${score}, difficulty: ${difficulty}`);
    return true;
  } catch (error) {
