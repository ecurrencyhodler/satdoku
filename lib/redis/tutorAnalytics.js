...
import { saveConversation, upsertMessageCounts, updateConversationLastMessage, markConversationCompleted, updateConversationStrategy } from '../supabase/chatAnalytics.js';
...
export async function trackConversationOpened(sessionId, gameVersion = null) {
  try {
    ...
    saveConversation({
      ...
      console.error('[tutorAnalytics] Failed to sync conversation to Supabase:', error);
      // Don't fail the Redis operation if Supabase fails
    });
...
export async function trackMessage(conversationId, sessionId, role) {
  try {
    ...
    // Only track user messages for analytics (stats page metric)
    // Assistant messages are not counted
    if (role !== 'user') {
      // Still update last_message_at for assistant messages to track conversation activity
      const conversationKey = `tutor_analytics:conversation:${conversationId}`;
      ...
      updateConversationLastMessage(conversationId, now.toISOString()).catch(error => {
        console.error('[tutorAnalytics] Failed to update last message timestamp:', error);
      });

      return { success: true };
    }

    // Increment message counters only for user messages
    ...
    // Get current message counts for Supabase sync (reuse existing keys)
    const [totalMessagesStr, userMessagesStr] = await Promise.all([
      redis.get(messagesKey),
      redis.get(userMessagesKey)
    ]);
    const totalMessages = totalMessagesStr ? parseInt(totalMessagesStr, 10) : 0;
    const userMessages = userMessagesStr ? parseInt(userMessagesStr, 10) : 0;

    // Sync to Supabase (fire-and-forget, non-blocking)
    // totalMessages and userMessages should be equal since we only track user messages
    Promise.all([
      upsertMessageCounts(conversationId, totalMessages, userMessages),
      updateConversationLastMessage(conversationId, now.toISOString())
    ]).catch(error => {
      console.error('[tutorAnalytics] Failed to sync message counts to Supabase:', error);
      // Don't fail the Redis operation if Supabase fails
    });
...
/**
 * Clear the current active conversation ID for a session
 * This allows a new conversation to be started
 * @param {string} sessionId - Session ID
 * @returns {Promise<boolean>} Success status
 */
export async function clearCurrentConversationId(sessionId) {
  try {
    if (!sessionId || typeof sessionId !== 'string') {
      return false;
    }
    ...
    const currentConversationKey = `tutor_analytics:current_conversation:${sessionId}`;
    await redis.del(currentConversationKey);
    return true;
  } catch (error) {
    console.error('[tutorAnalytics] Error clearing current conversation ID:', error);
    if (error.message?.includes('Connection') || error.message?.includes('ECONNREFUSED')) {
      resetRedisClient();
    }
    return false;
...
/**
 * Get count of paid conversations for current session
 * NOTE: Paid conversations are NOT keyed by gameVersion because version changes on every move.
 * They are cleared only when DELETE /api/tutor/chat-history is called (new game).
 * @param {string} sessionId - Session ID
 * @param {number} gameVersion - Game version (kept for API compatibility but not used for keying)
...
export async function getPaidConversationsCount(sessionId, gameVersion) {
  try {
    ...
    // Paid conversations persist across version changes (moves) within same game
    // Only reset when new game starts (via DELETE endpoint)
    const key = `tutor_chat_paid_conversations:${sessionId}`;
...
export async function incrementPaidConversations(sessionId, gameVersion) {
  try {
    ...
    console.log(`[tutorAnalytics] Incremented paid conversations: ${sessionId} -> ${newCount}`);
...
export async function canStartConversationWithoutPayment(sessionId, gameVersion) {
  try {
    ...
    if (gameVersion === null || gameVersion === undefined) {
      // If no game version, default to allowing (will fail elsewhere if needed)
      return true;
    }
...
/**
 * Mark current conversation as completed in Supabase
 * @param {string} sessionId - Session ID
 * @param {number} gameVersion - Game version
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function trackConversationCompleted(sessionId, gameVersion = null) {
  try {
    if (!sessionId || typeof sessionId !== 'string') {
      console.error('[tutorAnalytics] Invalid sessionId:', sessionId);
      return { success: false, error: 'Invalid sessionId' };
    ...
    if (!conversationId) {
      console.warn('[tutorAnalytics] No current conversation found to mark as completed');
      return { success: false, error: 'No current conversation' };
    }

    // Mark as completed in Supabase (fire-and-forget, non-blocking)
    markConversationCompleted(conversationId, sessionId, gameVersion).catch(error => {
      console.error('[tutorAnalytics] Failed to mark conversation as completed in Supabase:', error);
      // Don't fail the operation if Supabase fails
    });

    console.log(`[tutorAnalytics] Marked conversation as completed: ${conversationId}, session: ${sessionId}`);
    return { success: true, conversationId };
  } catch (error) {
    console.error('[tutorAnalytics] Error tracking conversation completed:', error);
    if (error.message?.includes('Connection') || error.message?.includes('ECONNREFUSED')) {
      resetRedisClient();
    }
    return { success: false, error: error.message };
...
/**
 * Track strategy selection for the current conversation
 * @param {string} sessionId - Session ID
 * @param {string} strategy - Strategy name
 * @returns {Promise<{success: boolean, conversationId?: string, error?: string}>}
 */
export async function trackStrategySelection(sessionId, strategy) {
  try {
    if (!sessionId || typeof sessionId !== 'string') {
      console.error('[tutorAnalytics] Invalid sessionId:', sessionId);
      return { success: false, error: 'Invalid sessionId' };
    ...
    if (!strategy || typeof strategy !== 'string') {
      console.error('[tutorAnalytics] Invalid strategy:', strategy);
    ...
    if (!conversationId) {
      console.warn('[tutorAnalytics] No current conversation found for session:', sessionId);
      return { success: false, error: 'No current conversation' };
    }

    // Update strategy in Supabase (non-blocking, only updates if strategy is null)
    updateConversationStrategy(conversationId, strategy).catch(error => {
      console.error('[tutorAnalytics] Failed to update conversation strategy in Supabase:', error);
      // Don't fail the Redis operation if Supabase fails
    });

    return { success: true, conversationId };
  } catch (error) {
    console.error('[tutorAnalytics] Error tracking strategy selection:', error);
    if (error.message?.includes('Connection') || error.message?.includes('ECONNREFUSED')) {
      resetRedisClient();
    }
    return { success: false, error: error.message };
