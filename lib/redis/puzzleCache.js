...
/**
 * Clear the puzzle cache for a specific difficulty or all difficulties
 * @param {string} [difficulty] - Difficulty level to clear, or undefined to clear all
 * @returns {Promise<{success: boolean, cleared: string[]}>} - Success status and cleared difficulties
 */
export async function clearPuzzleCache(difficulty = null) {
  try {
    const redis = await getRedisClient();
    if (!redis) {
      console.warn('[PuzzleCache] Cannot clear cache - Redis not available');
      return { success: false, cleared: [] };
    }

    const difficulties = difficulty ? [difficulty] : ['beginner', 'medium', 'hard'];
    const cleared = [];

    for (const diff of difficulties) {
      const listKey = `${CACHE_PREFIX}:${diff}`;
      const countKey = `${CACHE_COUNT_KEY}:${diff}`;
      
      // Delete both the list and count keys
      await redis.del(listKey);
      await redis.del(countKey);
      cleared.push(diff);
      console.log(`[PuzzleCache] Cleared cache for ${diff}`);
    }

    return { success: true, cleared };
  } catch (error) {
    console.error('[PuzzleCache] Error clearing cache:', error);
    if (error.message?.includes('Connection') || error.message?.includes('ECONNREFUSED')) {
      resetRedisClient();
    }
    return { success: false, cleared: [] };
