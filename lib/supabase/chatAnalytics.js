  ...
  try {
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        strategy: conversation.strategy || null,
      }, {
...
/**
 * Update strategy for a conversation (only if not already set)
 * @param {string} conversationId - Conversation ID
 * @param {string} strategy - Strategy name
 * @returns {Promise<boolean>} Success status
 */
export async function updateConversationStrategy(conversationId, strategy) {
  try {
    if (!conversationId || typeof conversationId !== 'string') {
      console.error('[chatAnalytics] Invalid conversationId:', conversationId);
      return false;
    ...
    if (!strategy || typeof strategy !== 'string') {
      console.error('[chatAnalytics] Invalid strategy:', strategy);
      return false;
    }

    const supabase = createSupabaseClient();

    // Only update if strategy is not already set (first strategy wins)
    const { error } = await supabase
      .from('tutor_conversations')
      .update({ strategy })
      .eq('conversation_id', conversationId)
      .is('strategy', null); // Only update if strategy is null

    if (error) {
      console.error('[chatAnalytics] Error updating conversation strategy:', error);
      return false;
    }
