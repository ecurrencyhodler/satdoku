...
export class GameValidation {
  ...
  static validatePlaceNumber(state, row, col, value) {
    ...
    // Check if cell already has the correct value (prevent repeated scoring)
    const currentValue = state.currentBoard[row][col];
    const correctValue = state.currentSolution[row][col];
    if (currentValue !== 0 && currentValue === correctValue && value === correctValue) {
      return { valid: false, error: 'Cell already contains the correct value', errorCode: 'INVALID_MOVE' };
    }
  ...
  /**
   * Check if puzzle is complete (all cells filled, optionally verify against solution)
   */
  static isPuzzleComplete(board, solution = null) {
    for (let row = 0; row < BOARD_SIZE; row++) {
      for (let col = 0; col < BOARD_SIZE; col++) {
        if (board[row][col] === 0) {
          return false;
        }
        // If solution is provided, verify the value matches
        if (solution && board[row][col] !== solution[row][col]) {
          return false;
        }
      }
    }
