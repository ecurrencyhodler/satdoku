...
/**
 * Note: There is a small race condition window between checking qualification and adding.
 * In practice, this is rare and acceptable. The cleanup step will remove entries beyond top 10.
 * For full atomicity, consider using Redis transactions (WATCH/MULTI/EXEC) or a Lua script,
 * but those add complexity and may not be supported in all Redis configurations.
 */
...
export async function addLeaderboardEntry(sessionId, score, username) {
  ...
  // Note: Small race condition window here - another request could add a higher score
  // between this check and the add. The cleanup step will handle this.
  ...
  // Verify the entry is still in top 10 after adding (handles race condition)
  // If another request added a higher score, this entry might be pushed out
  if (username && username.trim().length > 0) {
    ...
    for (const member of allMembersAfterAdd) {
      try {
        const data = JSON.parse(member);
        if (data.username && data.username.trim().length > 0) {
          entriesWithUsernames.push({ member, score: data.score });
        }
      } catch (e) {
        // Skip invalid entries
      }
    ...
    // Check if our entry is in top 10
    const ourEntry = entriesWithUsernames.find(e => {
      try {
        const data = JSON.parse(e.member);
        return data.sessionId === sessionId && data.score === score;
      } catch {
        return false;
      }
    });

    // If we have more than 10 entries with usernames and ours is not in top 10, remove it
    if (entriesWithUsernames.length > 10 && (!ourEntry || entriesWithUsernames.indexOf(ourEntry) >= 10)) {
      await client.zRem(key, JSON.stringify(entryData));
      throw new Error('Score does not qualify for leaderboard (another entry was added concurrently)');
    }
  }
